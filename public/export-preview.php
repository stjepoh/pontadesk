...
foreach (array_slice($contracts, 0, 8) as $row) { echo '<tr><td>'.htmlspecialchars((string)$row['client_name'], ENT_QUOTES, 'UTF-8').'</td><td>'.htmlspecialchars((string)$row['contract_name'], ENT_QUOTES, 'UTF-8').'</td><td>'.htmlspecialchars((string)$row['end_date'], ENT_QUOTES, 'UTF-8').'</td></tr>'; }
...
foreach (array_slice($workLogs, 0, 8) as $row) { echo '<tr><td>'.htmlspecialchars((string)$row['client_name'], ENT_QUOTES, 'UTF-8').'</td><td>'.htmlspecialchars((string)$row['work_date'], ENT_QUOTES, 'UTF-8').'</td><td>'.(int)$row['duration_minutes'].' min</td></tr>'; }
